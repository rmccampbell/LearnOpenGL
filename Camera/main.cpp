...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		int swidth = viewport[2], sheight = viewport[3];
		glm::mat4 projection = glm::perspective(glm::radians(45.f), float(swidth) / sheight, 0.1f, 100.f);

		float radius = 10.f;
		float camX = sin(glfwGetTime()) * radius;
		float camZ = cos(glfwGetTime()) * radius;
		glm::mat4 view = glm::lookAt(glm::vec3(camX, 0.f, camZ), glm::vec3(0.f), glm::vec3(0.f, 1.f, 0.f));
