...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//glm::vec3 lightColor(1.0f); // = glm::sin(float(glfwGetTime()) * glm::vec3(2.0f, 0.7f, 1.3f));
		// directional light
		...
		float aspect = viewport[3] ? float(viewport[2]) / float(viewport[3]) : 1.0f;
		glm::mat4 projection = glm::perspective(glm::radians(camera.Zoom), aspect, 0.1f, 100.f);
		...
		lampShader.use();
		lampShader.setVec3("lampColor", glm::vec3(1));
...
unsigned int loadTexture(const char *path, bool flip) {
	...
	if (data) {
		...
		GLenum format = formats[nChannels];
